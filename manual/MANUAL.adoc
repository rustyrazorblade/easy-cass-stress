...
cassandra-stress is a workload-centric stress tool for Apache Cassandra, written in Kotlin.
...
One of the goals of cassandra-stress is to provide enough pre-designed
...
workload, and want to test that. cassandra-stress allows you to customize a
...
The current version is {CASSANDRA_STRESS_VERSION}.
...
$ sudo apt update
$ sudo apt install cassandra-stress
```
...
[subs="attributes"]
```
$ curl -L -O "https://dl.bintray.com/rustyrazorblade/tlp-tools-tarball/cassandra-stress-{CASSANDRA_STRESS_VERSION}.tar
$ tar -xzf cassandra-stress-{CASSANDRA_STRESS_VERSION}.tar
```
...
This is currently the only way to use the latest version of cassandra-stress.  I'm working on getting the packages updated.
...
You can now run the stress tool via the `bin/cassandra-stress` script.  This is not the same script you'll be running if you've installed from a package or the tarball.
...
Either add the `bin` directory to your PATH or from within cassandra-stress
run the following command to execute 10,000 queries:

```
include::examples/cassandra-stress-keyvalue.txt[lines=1]
```
...
----
include::examples/cassandra-stress-keyvalue.txt[lines=2..-1]
----
...
You'll probably want to do a bit more than simply run a few thousand queries against a KeyValue table with default settings.  The nice part about cassandra-stress is that it not only comes with a variety of workloads that you can run to test your cluster, but that it allows you to change many of the parameters.  In the quickstart example we used the `-n` flag to change the total number of operations `cassandra-stress` will execute against the database.  There are many more options available, this section will cover some of them.

=== General Help

`cassandra-stress` will display the help if the `cassandra-stress` command is run without any arguments or if the `--help` flag is passed:

```bash
include::examples/cassandra-stress-help.txt[]
```
...
```
$ cassandra-stress run KeyValue -d "1h 30m"
```

To run a test for 1 days, 3 hours and 15 minutes (why not?), run cassandra-stress as follows:

```
$ cassandra-stress run KeyValue -d "1d 3h 15m"
```
...
```
$ cassandra-stress run KeyValue --field.keyvalue.value='book(20,40)`
```
...
`cassandra-stress` uses the https://logging.apache.org/[Log4J 2] logging framework.

You can find the default log4j config inhttps://github.com/rustyrazorblade/cassandra-stress/blob/main/src/main/resources/log4j2.yaml[`conf`, window="_blank"].  This should be suitable for most use cases.

To use your own logging configuration, simply set the shell variable `CASSANDRA_STRESS_LOG4J` to the path of the new logging configuration before running `cassandra-stress` to point to the config file of your choice.
...
The Debian package installs a basic configuration file to `/etc/cassandra-stress/log4j2.yaml`.


=== Exporting Metrics

cassandra-stress automatically runs an HTTP server exporting metrics in Prometheus format on port 9501.
...
`cassandra-stress` is a work in progress.  Writing a stress workload isn't documented yet as it is still changing.
