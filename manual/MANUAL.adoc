...
```sql
-- Show summary statistics for operation latencies by operation type
SELECT 
    op, 
    COUNT(*) as count,
    MIN(duration_ns)/1000000.0 as min_ms,
    MAX(duration_ns)/1000000.0 as max_ms,
    AVG(duration_ns)/1000000.0 as avg_ms,
    APPROX_PERCENTILE(duration_ns/1000000.0, 0.95) as p95_ms,
    APPROX_PERCENTILE(duration_ns/1000000.0, 0.99) as p99_ms
FROM 'output.parquet'
GROUP BY op
ORDER BY count DESC;

-- Show error counts and rates by second
SELECT 
    STRFTIME(start_time_ms/1000, '%Y-%m-%d %H:%M:%S') as second,
    COUNT(*) as total_ops,
    COUNT(*) FILTER (WHERE reason IS NOT NULL) as errors,
    (COUNT(*) FILTER (WHERE reason IS NOT NULL)::FLOAT / COUNT(*)::FLOAT) * 100 as error_rate_percent
FROM 'output.parquet'
GROUP BY second
ORDER BY second;
```
